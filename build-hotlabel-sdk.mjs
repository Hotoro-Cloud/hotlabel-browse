/**
 * Simple script to compile the HotLabel SDK TypeScript file to JavaScript
 * and place it in the public/js directory.
 * 
 * Run with: node build-hotlabel-sdk.mjs
 */
...
async function buildSdk() {
  try {
    ...
    try {
      await fs.access(publicJsDir);
    } catch (error) {
      console.log('Creating public/js directory...');
      await fs.mkdir(publicJsDir, { recursive: true });
    }

    // Create the SDK file directly in public/js
    console.log('Creating HotLabel SDK in public/js directory...');
...
class HotLabelSDK {
   ...
   */
  handleOptionSelection(taskId, optionIndex) {
...
`;
    ...
    await fs.writeFile(path.join(publicJsDir, 'hotlabel-sdk.js'), sdkContent);
    console.log('Successfully created HotLabel SDK in public/js/hotlabel-sdk.js');
  } catch (error) {
    console.error(`Error building HotLabel SDK: ${error.message}`);
  }
}
